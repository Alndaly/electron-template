...
export class ViewManager {
  ...
  setView = (id: string) => {
    ...
    newView.setAutoResize({ width: true, height: true, horizontal: true, vertical: true })
    if (view.title === 'root') {
      newView && newView.setBounds({ x: 0, y: 0, width: this.window.getBounds().width, height: this.window.getBounds().height })
    } else {
      newView && newView.setBounds({ x: 0, y: 38, width: this.window.getBounds().width, height: this.window.getBounds().height })
